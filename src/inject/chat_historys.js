...
const easyIDB = require('../lib/easyIDB.js');
...
class ChatHistorys {
  ...
  initEvent() {
    const target = angular.element('#chatArea .scroll-wrapper>.scroll-content')[0];
    const loadHisStatus = document.createElement('div');
    ...
    const $getHistory = document.createElement('p');
  ...
  saveHistory({ type, originMsg }) { // 保存聊天记录到indexDB
    ...
    const msg = new Object(originMsg);
    ...
    setTimeout(() => {
      ...
      if (/@@/.test(msg.FromUserName)) {
        // 群聊需要记录发送者的信息
        // 在群的memberlist里查找
        const info = msg.Content.match(/@(.*)?:<br\/>(.*)?/);
        const members = window._contacts[msg.FromUserName].MemberList;
  ...
  getHistory(user) {
    ...
    try {
      ...
      let chatsObj;
      const loadHisStatus = document.getElementById('loadHisStatus');
      if (user === 'filehelper') {
      ...
      const start = chatsObj.chats.length - chatsObj.get - 1;
      const end = start - 4 >= 0 ? start - 4 : 0;
      const his = chatsObj.chats;
      for (let i = start; i >= end; i -= 1) {
        ...
        if (/@@/.test(user)) {
          ...
          if (his[i].sendByLocal) {
            ...
            for (const member of window._contacts[user].MemberList) {
